...
import os
from jaxpi2.models import create_lr_schedule, create_optimizer, create_arch, create_train_state
...
from nc_porous import models
import pickle


def train_fvm(config, lb, ub, samplers):
    ...
    model = models.LidCavityPorous_fvm(config, lr, tx, arch, state, lb, ub)
    init_state = model.state  
    history = {'loss': []}    

    freq_pts = config.pseudo_time.update_schedule.get('every') 
    freq_loss = config.loss_weighting.update_schedule.get('every')
    freq_log = config.logging.get('log_every_steps')

...
def get_config():
    ...
    wandb.name = "NCPorousLayer_Ra-3.7e6_Da-1.534e-5_Pr-6.44_ffv" # Đổi tên cho đúng bài toán
    wandb.tag = None

    # =========================================================================
    # 2. Physics Parameters (Natural Convection - Table 2 Highest Ra)
    # =========================================================================
    ...
    physics.Re = 1.0       # Giữ nguyên để tránh lỗi chia 0
    physics.Pr = 6.44      # Chất lưu là Nước (thay vì 0.71 của không khí)
    physics.Ra = 3.7e6     # Rayleigh theo Case 1
    physics.Da = 1.534e-5   # Darcy theo Case 1
    physics.Je = 1.0       # Tỉ số nhớt động học (giữ nguyên)
    
    # Các thông số môi trường xốp (Porous Layer)
    # Bài báo dùng random packing of spherical beads, độ xốp thường rơi vào khoảng 0.36 - 0.4
    physics.epsilon = 0.4  
    physics.F_eps = 0.554  # [QUAN TRỌNG] BẬT lực Forchheimer theo đúng Table 4
    ...
    fvm_setup.h_stencil = 0.01 
    fvm_setup.alpha_u = 0.8 
    fvm_setup.alpha_p = 0.8
    fvm_setup.mode = False 
    ...
    ablation.use_sequential = False    # BẬT để dùng Scale-PINN
    ablation.use_laplacian = False     # BẬT để dùng Laplacian smoothing
    ablation.tau_mode = "fixed"       # Scale-PINN gốc dùng fixed tau
    ...
    optim.decay_steps = 5000     # Tăng step giảm LR vì bài toán hội tụ lâu hơn
    ...
    training.max_steps = 100000   # Ra=1e6 cần nhiều bước hơn để hình thành lớp biên sắc nét
    training.bs_eq = 128   
    training.bs_bc = 256        
    ...
    config.arch_input_dim = 3 # one more for HarBC embedding
    config.input_dim = 2
    config.seed = 42
